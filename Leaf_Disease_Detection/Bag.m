function [itrfin] = Bag( T,C,test )
%Inputs: T = Training Matrix, C = label, test = Testing matrix
...
for tempind=1:itrind
    ...
    if(N>2)
        ...
        %This while loop is the multiclass Trick
            c1=(C==u(itr));
            newClass=c1;
            
            
            t = templateTree('MaxNumSplits', 124);
            Bag = fitcensemble(T,newClass,'Method','Bag',...
                'NumLearningCycles', 30,'Learners',t,'CrossVal','on');
            
            classes = predict(Bag,tst);
